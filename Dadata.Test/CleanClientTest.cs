...
namespace Dadata.Test
{
    ...
    public class CleanClientTest
    {
        ...
        [Test]
        public async Task CleanAsyncAsIsTest()
        {
            var cleaned = await api.CleanAsync<AsIs>("Омномном");
            Assert.AreEqual(cleaned.source, "Омномном");
        }
        ...
        [Test]
        public async Task CleanAsyncAddressTest()
        {
            var cleaned = await api.CleanAsync<Address>("Москва Милютинский 13");
            Assert.AreEqual(cleaned.street, "Милютинский");
            Assert.AreEqual(cleaned.qc, "0");
        }
        ...
        [Test]
        public async Task CleanAsyncBirthdateTest()
        {
            var cleaned = await api.CleanAsync<Birthdate>("12.03.1990");
            Assert.AreEqual(cleaned.birthdate, new DateTime(1990, 3, 12));
            Assert.AreEqual(cleaned.qc, "0");
        }
        ...
        [Test]
        public async Task CleanAsyncEmailTest()
        {
            var cleaned = await api.CleanAsync<Email>("anderson@matrix.ru");
            Assert.AreEqual(cleaned.email, "anderson@matrix.ru");
            Assert.AreEqual(cleaned.qc, "0");
        ...
        [Test]
        public async Task CleanAsyncNameTest()
        {
            var cleaned = await api.CleanAsync<Fullname>("Ольга Викторовна Раздербань");
            Assert.AreEqual(cleaned.name, "Ольга");
            Assert.AreEqual(cleaned.qc, "0");
        }
        ...
        [Test]
        public async Task CleanAsyncPhoneTest()
        {
            var cleaned = await api.CleanAsync<Phone>("89168459285");
            Assert.AreEqual(cleaned.number, "8459285");
            Assert.AreEqual(cleaned.qc, "0");
        }
        ...
        [Test]
        public async Task CleanAsyncPassportTest()
        {
            var cleaned = await api.CleanAsync<Passport>("4506 629672");
            Assert.AreEqual(cleaned.series, "45 06");
            Assert.AreEqual(cleaned.qc, "10");
        }
        ...
        [Test]
        public async Task CleanAsyncVehicleTest()
        {
            var cleaned = await api.CleanAsync<Vehicle>("форд фокус");
            Assert.AreEqual(cleaned.brand, "FORD");
            Assert.AreEqual(cleaned.qc, "0");
        }
        ...
        [Test]
        public async Task CleanAsyncTest()
        {
            var structure = new List<StructureType> { StructureType.NAME, StructureType.ADDRESS };
            var data = new List<string> { "Кузнецов Петр Алексеич", "Москва Милютинский 13" };

            var cleaned = await api.CleanAsync(structure, data);
            Assert.AreEqual(cleaned.Count, 2);
